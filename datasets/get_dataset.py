...
from datasets.flow_datasets import Oceano
...
def get_dataset(all_cfg):
    ...
    elif cfg.type == 'OCEANO':
        train_set = Oceano(cfg.root_sintel_raw, n_frames=cfg.train_n_frames,
                              transform=input_transform, co_transform=co_transform)
        valid_input_transform = copy.deepcopy(input_transform)
        valid_input_transform.transforms.insert(0, sep_transforms.Zoom(*cfg.test_shape))
        valid_set_1 = Oceano(cfg.root_sintel, n_frames=cfg.val_n_frames,
                             transform=input_transform,co_transform=co_transform
                             )
        valid_set_2 = Oceano(cfg.root_sintel, n_frames=cfg.val_n_frames, 
                             transform=input_transform,co_transform=co_transform
